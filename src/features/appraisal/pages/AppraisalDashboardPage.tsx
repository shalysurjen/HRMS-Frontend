...
function resolveApproverLevel(
  ...
  const isSoleApprover = isL1 && !s.finalApproverId;

  if (isViewAll && !isL1 && !isL2) return "VIEW_ONLY";
  if (role === "CFO") return "VIEW_ONLY";

  if (isSoleApprover && s.status === "FINAL_REVIEW") return "L2";
...
function resolveTab(
  ...
  const level = resolveApproverLevel(s, userId, role, isViewAll);
...
const AppraisalDashboardPage = () => {
  ...
  if (selected && user?.id) {
    const level = resolveApproverLevel(selected, user.id, role, isViewAll);
    return (
      <AppraisalReviewPage
  ...
  const bucketed = user?.id
    ? summaries.reduce<Record<TabKey, AppraisalSummary[]>>(
        (acc, s) => {
          const t = resolveTab(s, user.id, role, isViewAll);
          acc[t].push(s);
          acc["ALL"].push(s);
          return acc;
        },
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 space-y-6">
      ...
      <div className="space-y-3">
        {displayed.map(s => {
          const level = user?.id ? resolveApproverLevel(s, user.id, role, isViewAll) : "VIEW_ONLY";
